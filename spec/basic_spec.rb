describe "BasicSpec" do
  ...
  it "should render graph objects in JSON" do
    ...
    graph.add_edge!({id: 1, data: {level: "1"}}, {id: 2, data: {level: 8}}, 14)
    expect(graph.nodes[1].to_json).to eq("{\"id\":1,\"data\":{\"level\":\"1\"}}")
    expect(graph.nodes[2].to_json).to eq("{\"id\":2,\"data\":{\"level\":8}}")
    expect(graph.edges["1==2"].to_json).to eq("{\"id\":\"1==2\",\"weight\":14}")
  end
